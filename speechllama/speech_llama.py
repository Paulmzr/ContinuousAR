...
class SpeechLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            labels = labels.reshape(bsz * speech_len, -1).repeat(self.diffusion_batch_mul, 1)
            z = z.reshape(bsz * speech_len, -1).repeat(self.diffusion_batch_mul, 1)
            mask = speech_attention_mask.reshape(bsz * speech_len).repeat(self.diffusion_batch_mul)
            loss = self.diffloss(z=z, target=labels, mask=mask)
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            # token selection
            if do_sample:
                next_embeds = self.diffloss.sample(next_token_logits, temperature=1.0) # bsz, dim
