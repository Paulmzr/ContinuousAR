...
class ScoreLossZ(nn.Module):
    ...
    def forward(self, target, z, mask=None, additional_targets=None):
        noise_1 = 2 * torch.rand((z.shape[0], self.noise_channels), dtype=z.dtype, device=z.device) - 1
        #noise_1 = torch.randn((z.shape[0], self.noise_channels), dtype=z.dtype, device=z.device)
        sample_1 = self.net(noise_1, z)
        noise_2 = 2 * torch.rand((z.shape[0], self.noise_channels), dtype=z.dtype, device=z.device) - 1
    ...
    def sample(self, z, temperature=1.0, cfg=1.0):
        if cfg != 1.0:
            ...
            z = z_1 * cfg + (1 - cfg) * z_2
        noise = 2 * torch.rand((z.shape[0], self.noise_channels), dtype=z.dtype, device=z.device) - 1
        #noise = torch.randn((z.shape[0], self.noise_channels), dtype=z.dtype, device=z.device)
        return self.net(noise, z).float()
        
        #noise = torch.randn((z.shape[0] // 2, self.noise_channels), dtype=z.dtype, device=z.device).repeat(2,1)
        #out = self.net(noise, z).float()
        #o_1, o_2 = out.chunk(2, dim=0)
        #o = o_1 * cfg + (1 - cfg) * o_2
        #return o
