...
class ModelArguments:
    ...
    tokenizer: str = "/data/mazhengrui/SpeechLLaMA/tokenizer_phoneme_librispeech"
    scale:str = "/data/mazhengrui/SpeechLLaMA/scale_encodec/librispeech/scale_10k"
...
@dataclass
class DataArguments:
    data_path: str = "/data/mazhengrui/librispeech_phoneme"
    train_split: Union[str, List[str]] = field(default_factory=lambda: "train.960")
    eval_split: Union[str, List[str]] = field(default_factory=lambda: "validation")
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    if training_args.do_train:
        #train_dataset = load_dataset(data_args.data_path, split=data_args.train_split)
        train_dataset = load_from_disk(data_args.data_path)[data_args.train_split]
    ...
    if training_args.do_eval:
        #eval_dataset = load_dataset(data_args.data_path, split=data_args.eval_split)
        eval_dataset = load_from_disk(data_args.data_path)[data_args.eval_split]
...
def train(attn_implementation="flash_attention_2"):
    ...
    '''
    tokenizer = PreTrainedTokenizerFast.from_pretrained(model_args.tokenizer, padding_side="left")
