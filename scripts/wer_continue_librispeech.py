...
def filter_function(example):
    return ((len(example["audio"]["array"]) / SAMPLING_RATE) < 10.0) and ((len(example["audio"]["array"]) / SAMPLING_RATE) > 4.0)


# 加载 Conformer-Transducer 模型
...
for sample_id, reference_text in tqdm(zip(ids, texts), total=len(ids)):
    ...
    # 使用 Conformer-Transducer 转录音频
    #transcription = model.transcribe(audio_path)
    transcription = model.transcribe(audio_path, verbose=False)

    predicted_text = transcription[1][0].strip()
