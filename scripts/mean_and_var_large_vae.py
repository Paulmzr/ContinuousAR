...
def main():
    ...
    model_path = '/data/mazhengrui/codec/descript-audio-codec/runs/vae-2/200k/vae/weights.pth'
    codec = dac.VAE.load(model_path)
    codec = codec.to(device).to(torch_dtype)
    codec.eval()
    ...
    with torch.no_grad():
        for i in tqdm(range(num_batches)):
            ...
            #print(audio_inputs["input_values"].size())    
            speech_inputs_embeds = codec.encode(audio_inputs["input_values"].to(torch_dtype).to(device)).sample() #b,d,t
            ...
            speech_inputs_embeds = speech_inputs_embeds.reshape(-1, speech_inputs_embeds.size(-1)) # b*t, d
            ...
            del selected_embeds, speech_inputs_embeds, audio_inputs
    ...
    torch.save(mean, './vae_scale/scale_10k/mean.pt')
    torch.save(std, './vae_scale/scale_10k/std.pt')
